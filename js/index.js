...
function resizePanel() {
	"use strict";
	var width = $(window).width(), height = $(window).height(), mask_width = width * $('.item').length;
...
var name_bubble_function = function (bubbleFlag) {
	...
	var bubbleOnce = setTimeout(fadeBubble, 7000);
}


function portfolioModel() {
	var project_data, currentSelections, self = this;
	...
	self.navigate = function (item) {
		...
		case "About" : 	$('#wrapper').scrollTo(
						...
						break;
		case "Experience" : $("#experience_tile").click();
							break;
		case "Education" : $("#education_tile").click();
						   break;
		case "Projects" : $("#projects_tile").click();
						  break;
		case "Achievements" : $("#achievements_tile").click();
							  break;
		case "Contact" : $("#contact_tile").click();
						 break;
		}
	...
	self.getExperienceContent = function() {
		$.getJSON('/experience', function(data) {
			...
			(function change () {
				...
				$('#experience_tile_wrapper').scrollTo(
						$(item_id+""), 
						900, { 
							easing: 'swing' 
						}
	...
	self.getEducationContent = function () {
		$.getJSON('/education', function (data) {
			var i = 0, edu_data = "", cnt = data.education.length, pc1 = cnt * 100, pc2 = (100 / cnt);
			...
			(function change () {
				...
				$('#education_tile_wrapper').scrollTo(
						$(item_id+""), 
						900, {
							easing: 'swing' 
						}
				);								
				setTimeout(change, 3300);					
			})();			
		});
	}
	...
	self.get_github_activities = function () {
		...
		$.getJSON('/github_activity', function (data) {
			var i = 0, entries = data.activities, cnt = data.activities.length, pc1 = cnt * 100, pc2 = (100 / cnt);
			...
			(function change(){
				...
				$('#activity_tile_wrapper').scrollTo(
						$(item_id+""), 
						2000, {	
							easing: 'swing' 
						}
	...
	self.get_projects_info = function () {
		...
		$.getJSON('/projects', function (data) {		
			var proj_cat = [], i = 0, cnt = data.projects.length, pc1 = (cnt+2) * 100, pc2 = (100 / cnt);
			...
			(function change(){
				...
				$('#projects_tile_wrapper').scrollTo(
						$(item_id+""), 
						900, {	
							easing: 'swing' 
						}
				);				
				setTimeout(change,5650);
			})();
	...
	self.getDetails = function (item) {
		...
		$('#projects_detail_wrapper').scrollTo( 
				$("" + itemId), 
				2600, 
				{
					easing: 'swing'
